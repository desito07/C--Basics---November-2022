...
namespace Exams_Prep
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            double prepTeren = timeScreening * 0.15;
            ...
            {
                Console.WriteLine($"Time is up! To complete the movie you need {Math.Round(totalTime - timeScreening)} minutes.");
            }
            else
            {
                Console.WriteLine($"You managed to finish the movie on time! You have {Math.Round(timeScreening - totalTime)} minutes left!");
            }
        }
    }
}
